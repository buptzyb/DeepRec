...
from tensorflow.python.training.experimental.gpu_offload import make_gpu_offload
import os
import sys
import math
import collections
    ...
    def _dnn(self, dnn_input, dnn_hidden_units=None, layer_name=''):
        for layer_id, num_hidden_units in enumerate(dnn_hidden_units):
            with tf.variable_scope(layer_name + '_%d' % layer_id,
                ...
                weights = create_offload_var(lambda: tf.glorot_uniform_initializer()([dnn_input.shape[1], num_hidden_units]))
                biases = create_offload_var(lambda: tf.zeros([num_hidden_units]))
                dnn_input = tf.matmul(dnn_input, weights) + biases
                dnn_input = self._dnn_activation(dnn_input, name=dnn_layer_scope.name)
                if self.use_bn:
                    scale = create_offload_var(lambda: tf.ones([num_hidden_units]))
                    offset = create_offload_var(lambda: tf.zeros([num_hidden_units]))
                    pop_mean = create_offload_var(lambda: tf.zeros([num_hidden_units]))
                    pop_variance = create_offload_var(lambda: tf.ones([num_hidden_units]))
                    epsilon = 1e-3
                    dnn_input = tf.nn.batch_normalization(dnn_input, pop_mean, pop_variance, offset, scale, epsilon)
    ...
    def cgc_model(self, inputs, level_name, is_last=False):
        ...
        with tf.cuda.stream_scope(tf.cuda.get_stream(3)):
            ...
            for i in range(self._num_tasks):
                ...
                weights = create_offload_var(lambda: tf.glorot_uniform_initializer()([gate_input.shape[1], cur_expert_num]))
                gate_out = tf.matmul(gate_input, weights, name=level_name + 'gate_softmax_specific_' + self._towers[i][0])
                gate_out = tf.expand_dims(gate_out, axis=-1)

                gate_mul_expert = tf.math.reduce_sum(expert_concat * gate_out, axis=1, keep_dims=False,
        ...
        with tf.cuda.stream_scope(tf.cuda.get_stream(4)):
            if not is_last:
                ...
                weights = create_offload_var(lambda: tf.glorot_uniform_initializer()([gate_input.shape[1], cur_expert_num]))
                gate_out = tf.matmul(gate_input, weights)
                gate_out = tf.nn.softmax(gate_out, name=level_name + 'gate_softmax_shared')
                gate_out = tf.expand_dims(gate_out, axis=-1)
...
def main(tf_config=None, server=None):
    ...
    # sess_config.gpu_options.stream_merge_options.merge_d_to_h_stream = True
    # sess_config.gpu_options.stream_merge_options.merge_d_to_d_stream = True
    # sess_config.gpu_options.multi_stream_options.stream_group_count = 5
    # sess_config.gpu_options.multi_stream_options.gpu_memory_pool_mode = "shared"
    # sess_config.gpu_options.multi_stream_options.host_allocator_mode = "private"
