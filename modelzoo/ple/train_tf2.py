...
class PLE():
    ...
    def _dnn(self, dnn_input, dnn_hidden_units=None, layer_name=''):
        for layer_id, num_hidden_units in enumerate(dnn_hidden_units):
            with tf.variable_scope(layer_name + '_%d' % layer_id,
                ...
                weights = tf.Variable(tf.glorot_uniform_initializer()([dnn_input.shape[1], num_hidden_units]))
                ...
                if self.use_bn:
                    scale = tf.Variable(tf.ones([num_hidden_units]))
                    offset = tf.Variable(tf.zeros([num_hidden_units]))
                    pop_mean = tf.Variable(tf.zeros([num_hidden_units]))
                    pop_variance = tf.Variable(tf.ones([num_hidden_units]))
                    epsilon = 1e-3
    ...
    def cgc_model(self, inputs, level_name, is_last=False):
        ...
        for i in range(self._num_tasks):
            ...
            weights = tf.Variable(tf.glorot_uniform_initializer()([gate_input.shape[1], cur_expert_num]))
        ...
        with self._make_scope('PLE', self._bf16, self._dense_layer_partitioner):
            ...
            for i, tower in enumerate(self._towers):
                ...
                with tf.variable_scope(tower_name, reuse=tf.AUTO_REUSE):
                    ...
                    weights = tf.Variable(tf.glorot_uniform_initializer()([tower_output.shape[1], 1]))
